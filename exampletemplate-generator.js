...
function render_exampletemplate_from_json_ld_file(filename, outputdirectory) {
  ...
  .then(
	  ...
	  classes = parsed_json.classes;
	  for (i in classes) {
		var filenamei = outputdirectory + '/' + camelCase(classes[i].name.nl) + '.json';
	  ...
	  for (i in classes) {
		var filenamei = outputdirectory + '/context/' + camelCase(classes[i].name.nl) + '.jsonld';
