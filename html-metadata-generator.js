...
program.on("--help", function () {
  ...
  console.log(
    "  $ html-metadata-generator -m <mainlanguage> -i <input> -o <output>",
  );
});
  ...
  jsonfile
    ...
    .then(function (input) {
      ...
      jsonfile.writeFile(output_filename, output, function (err) {
        ...
        console.log(
          prefix + "The file has been saved to " + output_filename + newLineMd,
        );
      });
    })
    .catch((error) => {
...
function make_nj_metadata(json, hostname, language, prefix) {
  ...
  if (json.navigation) {
    ...
    console.log(
      "Warning: no navigation defined for this rendering" + newLineMd,
    );
    json.navigation = {
      self: hn + json.urlref,
    };
  ...
  let translationObj = json.translation?.find(
  ...
  if (
    ...
    languageNames[primaryLanguage] !== undefined
  ) {
